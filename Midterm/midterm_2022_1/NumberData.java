...
public abstract class NumberData {
    ...
    public void stirNumbers() {
        for (int i = 0; i < 10; i++) { // 10번 실행
            int randnum1 = numGenerator.nextInt(10);
            int randnum2 = numGenerator.nextInt(10);
            if (randnum1 != randnum2){  // 두 임의의 수가 같지 않을 때만 의미가 있으므로
                                        // 서로 다를 때, Swap.
                int temp = numbers[randnum1];
                numbers[randnum1] = numbers[randnum2];
                numbers[randnum2] = temp; 
            }
    ...
    public void stirSortPrintData(String algName) {
        stirNumbers();
        printNumbers(); // 섞은 후 출력
        //정렬 알고리즘 생성
        sortAlg = createSortAlg(algName);
        sort();
        printNumbers();
    }

    public abstract SortAlg createSortAlg(String sorter);
}
